package com.google.kpierudzki.driverassistant.geoSamples.database;
...
public interface GeoSamplesDao {
    ...
    @Query("SELECT * FROM " + GeoSamplesTracksEntity.TABLE_NAME +
            " ORDER BY " + GeoSamplesTracksEntity.START_TIME_IN_MILLIS_COL +
            " DESC LIMIT :n")
    List<GeoSamplesTracksEntity> getLastNTracks(int n);

    @Query("SELECT * FROM " + GeoSamplesTracksEntity.TABLE_NAME + " WHERE " +
            GeoSamplesTracksEntity.START_TIME_IN_MILLIS_COL + " BETWEEN :startTimeInMillis AND :endTimeInMillis ORDER BY "
            + GeoSamplesTracksEntity.START_TIME_IN_MILLIS_COL + " DESC")
    List<GeoSamplesTracksEntity> getTracksBetween(long startTimeInMillis, long endTimeInMillis);

    @Query("SELECT "
            + GeoSamplesEntity.TABLE_NAME + "." + GeoSamplesEntity.SPEED_MPS2 + " AS speed, "
            + EcoDrivingEntity.TABLE_NAME + "." + EcoDrivingEntity.CURRENT_ACCELERATION_COL_MPMS2 + " AS currentAcceleration, "
            + " 0 AS avgScore FROM "
