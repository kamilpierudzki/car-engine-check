    ...
    public void onTrackSelected(HistoryCalendarContract.CalendarTranslateInfoModel track) {
        if (getActivity() != null) {
            getActivity().getSupportFragmentManager().beginTransaction()
                    .replace(
                            mainActivityFragmentsCallbacks.getRootContainer().getId(),
                            HistoryMapFragment.newInstance(
                                    new HistoryMapContract.MapInfoModel(
                                            track.trackId,
                                            track.startPointTranslation,
                                            track.endPointTranslation,
                                            track.startTimeInMillis)))
                    .addToBackStack(null)
                    .commit();
        }
    }
