...
open class BaseServiceManager(connectorTypes: Array<ManagerConnectorType>) : BackgroundThreadPoolHelper(), IConnectorSelectable {
    ...
    public val listeners: ArrayList<IBaseManager.IBaseManagerListener> = ArrayList()
    ...
    @WorkerThread
    inline fun <reified T, R> notifyListenersWithData(data: R, method: (data: R, type: T) -> Unit) {
        synchronized(this) {
            listeners.filter { it is T }.map { it as T }.forEach { method.invoke(data, it) }
        }
    }
}